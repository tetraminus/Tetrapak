    ...
    SMODS.Centers[tpjokerSlug("bad_star_chart")].calculate = function(self, card, context)
        if context.using_consumeable then
            ...
            if consumeable.ability.set == "Planet" then 
                ...
                update_hand_text({sound = 'button', volume = 0.7, pitch = 0.8, delay = 0.3}, {handname=localize(consumeable.ability.consumeable.hand_type, 'poker_hands'),chips = G.GAME.hands[consumeable.ability.consumeable.hand_type].chips, mult = G.GAME.hands[consumeable.ability.consumeable.hand_type].mult, level=G.GAME.hands[consumeable.ability.consumeable.hand_type].level})
